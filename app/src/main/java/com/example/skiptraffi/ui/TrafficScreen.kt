...
fun TrafficScreen(viewModel: TrafficViewModel, navController: NavController) {
    ...
    viewModel.trafficAreas?.let { TrafficAreaList(trafficAreaList = it, navController) }
}

@Composable
fun TrafficAreaList(trafficAreaList: List<Area>, navController: NavController) {
...
@Composable
fun CurrentPositionItem(onClick: (Int) -> Unit) {
    Card(
        modifier = Modifier
            .padding(8.dp, 4.dp)
            .fillMaxWidth()
